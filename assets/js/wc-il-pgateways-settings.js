...
;(function ( $, window, document ) {
	...
	var uploadField = {
		frames: [],
		init: function() {
			$( 'button.ilpg_image_upload' )
				.on( 'click', this.onClickUploadButton );

			$( 'button.ilpg_image_remove' )
		...
		onSelectAttachment: function() {
			...
			$field.siblings( 'button.ilpg_image_remove' ).show();
			$field.siblings( 'button.ilpg_image_upload' ).hide();
		},
		...
		removeProductImage: function( event ) {
			...
			$button.hide();
			$field.siblings( 'button.ilpg_image_upload' ).show();
		}
	};
